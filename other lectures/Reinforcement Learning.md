...
$$
\text{arg}\min_\omega\frac{1}{2N}\sum_{i=1}^N[Q_\omega(s_i,a_i)-(r_i+\gamma\max_{a'\in\mathcal{A}}Q_\omega(s_{i+1},a'))^2]
$$
...
在实际的实现过程中，我们一般会使用两个网络。一个，是实时更新的网络，我们称之为训练网络；一个，是每隔$C$步才会和训练网络参数进行一次同步的网络。我们称之为目标网络。在计算损失函数的过程中，我们使用目标网络的参数；在更新的过程中，我们更新训练网络的参数。
