...
组相联是一种相对折中的方式。主存中的每个数据块，会通过之前的类似于索引的方式，存放到对应的组中。但是，在组内，数据块的存放方式是任意的。如果一个组(index)中可以存放n个数据块，那么我们称这是一个n路相联cache。因此，为了查找一个数据块，我们必须检查某个组中所有元素的标签。如果相联度越高，那么失效率会降低，但是命中时间会增长。因此，我们的主要目标就是要找到失效率和命中时间的平衡。
...
- 组相联cache中，数据返回在决定命中/缺失之后；而直接映射中，则可以在此之前。

- 通过 Victim Cache 降低缓存缺失。也就是添加了一个 buffer, 用于存放那些即将被移除的缓存块。

- Pseudo-Associativity: 把一个 2-way cache分成两半，一半是默认的部分，如果数据在这里就是 fast hit; 如果没有命中，那么检查另一半，如果数据命中，就是 slow hit(pseudo-hit)，否则才是缺失。

- Prefetching:把数据块提前放到 stream buffer 中。
...
- 页着色：要求别名与原本的块的最后多少位是相同的，这样可以保证在如直接映射的缓存中，相同的块不会在cache中多次出现。
